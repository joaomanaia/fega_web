...
const GroupMessageContent: React.FC<GroupMessageContentType> = ({
  ...
  return (
    ...
    >
      ...
      {addUserPopupVisible && (
        <AddUserGroupPopup groupId={group.id} onUserAdded={() => setAddUserPopupVisible(false)} />
      )}
      {removeUserPopupVisible && (
        <RemoveUserGroupPopup
          groupId={group.id}
          onUserRemoved={() => setRemoveUserPopupVisible(false)}
        />
      )}
      <div
      ...
      >
        ...
        <img className="rounded-full h-10 w-10" src={group.groupImage} alt={group.groupName} />
      ...
      {!canSendMessage && (
        <p className="ml-4 text-black dark:text-white">Wait {sendMessageInterval} seconds</p>
      )}
